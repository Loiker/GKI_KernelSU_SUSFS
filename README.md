...
| O3 | 引入O3编译优化，改善内核运行性能 |
| 网络优化 | 加入 BBR/Brutal 及一系列 tcp 拥塞控制算法，网络连接性能优化 |
| IO调度器 | 三星SSG IO调度器移植 |
| Re:Kernel | Re:Kernel支持，与Freezer，NoActive等软件配合降低功耗 |
| 内核防格 | 内核防格基带保护(By [@showdo](https://github.com/showdo))，有效防止恶意格机脚本/程序对系统分区数据的破坏 |
